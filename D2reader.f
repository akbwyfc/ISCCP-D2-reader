...
C**********************************************************************C
C**********************************************************************C
      SUBROUTINE RDANC11(LUNANC,IRC)                                      
      PARAMETER     ( MAXBOX = 6596 )                                   
      COMMON /SQUARE/ LONLIM(2,MAXBOX)                                  
      CHARACTER*80    HEADER                                            
      OPEN(LUNANC,ACCESS='DIRECT',RECL=80,IOSTAT=IRC)   !,FORM='FORMATTED'
      IF ( IRC .NE. 0 ) RETURN                                          
      READ(LUNANC,REC=1) HEADER !,FMT='(A80)'
		print*,HEADER	  
      READ(LUNANC,REC=2) HEADER  !,FMT='(A80)'
		print*,HEADER,11113
		print*,111112
      DO 100 IREC=1,MAXBOX          !,FMT=110                                    
         READ(LUNANC,REC=IREC+2) IBOX,J,I,LONBEG,LONEND,   
     &       CENLAT,CENLON,IAREA,LANDFR,ITOPOG,IVEG 
	     print*,IBOX,J,I,LONBEG,LONEND
         PRINT*,11114
         LONLIM(1,IBOX) = LONBEG                                        
         LONLIM(2,IBOX) = LONEND                                        
  100 CONTINUE     
  110 FORMAT(I4) !,2F9.2,I8,I6,I7,I4)  
      RETURN                                                            
      END   
	...
	SUBROUTINE RDANC(LUNANC,IRC)                                      
      ...
      COMMON /SQUARE/ LONLIM(2,MAXBOX)
      CHARACTER*80    HEADER
	open(LUNANC,file='fort.9')
      READ(LUNANC,*) HEADER !,FMT='(A80)'
		print*,HEADER	  
      READ(LUNANC,*) HEADER  !,FMT='(A80)'
		print*,HEADER,11113,IRC
	do IREC=1,MAXBOX
	READ(LUNANC,*) IBOX,J,I,LONBEG,LONEND
     &,CENLAT,CENLON,IAREA,LANDFR,ITOPOG,IVEG
         LONLIM(1,IBOX) = LONBEG                                        
         LONLIM(2,IBOX) = LONEND 
	 end do	
